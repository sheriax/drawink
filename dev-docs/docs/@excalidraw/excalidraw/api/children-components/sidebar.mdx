...
The editor comes with a default sidebar on the right in LTR (Left to Right) mode which contains the library. You can also add your own custom sidebar(s) by rendering this component as a child of `<Drawink>`.
...
Further, if the sidebader doesn't comfortably fit in the editor, it won't be dockable. To decide the breakpoint for docking you can use [UIOptions.dockedSidebarBreakpoint](/docs/@drawink/drawink/api/props/ui-options#dockedsidebarbreakpoint).
...
You can use the [`ref.toggleSidebar({ name: "custom" })`](/docs/@drawink/drawink/api/props/drawink-api#toggleSidebar) api to control the sidebar, but we export a trigger button to make UI use cases easier.
