...
Earlier we were using `renderFooter` prop to render custom footer which was removed in [#5970](https://github.com/drawink/drawink/pull/5970). Now you can pass a `Footer` component instead to render the custom UI for footer.
...
For `mobile` you will need to render it inside the [MainMenu](#mainmenu). You can use the [`useEditorInterface`](#useEditorInterface) hook to check the type of device, this will be available only inside the `children` of `Drawink` component.
