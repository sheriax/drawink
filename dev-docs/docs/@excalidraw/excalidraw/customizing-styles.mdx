...
Drawink uses CSS variables to style certain components. To override them, you should set your own on the `.drawink` and `.drawink.theme--dark` (for dark mode variables) selectors.
...
For a complete list of variables, check [theme.scss](https://github.com/drawink/drawink/blob/master/packages/drawink/css/theme.scss), though most of them will not make sense to override.

```css showLineNumbers
.custom-styles .drawink {
...
.custom-styles .drawink.theme--dark {
