...
const percentages = fs.readFileSync(
  `${__dirname}/../packages/drawink/locales/percentages.json`,
);
...
const printRow = (id, locale, coverage) => {
  ...
  if (locale in crowdinMap && crowdinMap[locale]) {
    result += `[${boldIf(
      language,
      isOver,
    )}](https://crowdin.com/translate/drawink/10/${crowdinMap[locale]}) | `;
  } else {
...
console.info(
  `Each language must be at least **${THRESSHOLD}%** translated in order to appear on Drawink. Join us on [Crowdin](https://crowdin.com/project/drawink) and help us translate your own language. **Can't find yours yet?** Open an [issue](https://github.com/drawink/drawink/issues/new) and we'll add it to the list.`,
);
console.info("\n\r");
