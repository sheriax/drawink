...
This prop can be used to customise UI of Drawink. Currently we support customising [`canvasActions`](#canvasactions), [`dockedSidebarBreakpoint`](#dockedsidebarbreakpoint) [`welcomeScreen`](#welcmescreen) and [`tools`](#tools).

<pre>
  &#123;
  <br /> canvasActions?: <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L372">
    CanvasActions
...
| `toggleTheme` | `boolean` &#124; `null` | `null` | Indicates whether to show `Theme toggle`. When defined as `boolean`, takes precedence over [`props.theme`](/docs/@drawink/drawink/api/props#theme) to show `Theme toggle`. |
...
This prop indicates at what point should we break to a docked, permanent sidebar. If not passed it defaults to [`MQ_RIGHT_SIDEBAR_MAX_WIDTH_PORTRAIT`](https://github.com/drawink/drawink/blob/master/packages/drawink/constants.ts#L161).  
If the _width_ of the _drawink_ container exceeds _dockedSidebarBreakpoint_, the sidebar will be `dockable` and the button to `dock` the sidebar will be shown  
...
function App() {
  return (
    <div style={{ height: "500px" }}>
      <Drawink UIOptions={{dockedSidebarBreakpoint: 200}}/>
    </div>
  );
}
```
