...
<pre>
  (api:{" "}
  <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L616">
    DrawinkAPI
  </a>
  ) => void;
</pre>
...
export default function App() {
  const [drawinkAPI, setDrawinkAPI] = useState(null);
  return <Drawink drawinkAPI={(api)=> setDrawinkAPI(api)} />;
}
```

You can use this prop when you want to access some [Drawink APIs](https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L616). We expose the below APIs :point_down:
...
| [refresh](#refresh) | `function` | Updates the offsets for the Drawink component so that the coordinates are computed correctly (for example the cursor position). |
| [setToast](#settoast) | `function` | This API can be used to show the toast with custom message. |
| [id](#id) | `string` | Unique ID for the drawink component. |
...
:::info The `Ref` support has been removed in v0.17.0 so if you are using refs, please update the integration to use the `drawinkAPI`.
...
<pre>
  (scene:{" "}
  <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L339">
    sceneData
  </a>
  ) => void
</pre>
...
| --- | --- | --- |
| `elements` | [`ImportedDataState["elements"]`](https://github.com/drawink/drawink/blob/master/packages/drawink/data/types.ts#L38) | The `elements` to be updated in the scene |
| `appState` | [`ImportedDataState["appState"]`](https://github.com/drawink/drawink/blob/master/packages/drawink/data/types.ts#L39) | The `appState` to be updated in the scene. |
| `collaborators` | <code>Map<string, <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L37">Collaborator></a></code> | The list of collaborators to be updated in the scene. |
| `captureUpdate` | [`CaptureUpdateAction`](https://github.com/drawink/drawink/blob/master/packages/drawink/store.ts#L40) | Controls which updates should be captured by the `Store`. Captured updates are emmitted and listened to by other components, such as `History` for undo / redo purposes. |
...
function App() {
  ...
  return (
    <div style={{ height: "500px" }}>
      ...
      <Drawink drawinkAPI={(api) => setDrawinkAPI(api)} />
    </div>
  );
}
```
...
<pre>
  ...
  <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L249">
    LibraryItemsSource
  </a>
  ;<br /> merge?: boolean; <br /> prompt?: boolean;
  <br /> openLibraryMenu?: boolean;
  <br /> defaultStatus?: "unpublished" | "published"; <br /> &#125;) => Promise&lt;
  <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L246">
    LibraryItems
  </a>
  &gt;
</pre>
...
| --- | --- | --- | --- |
| `libraryItems` | [LibraryItemsSource](https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L249) | \_ | The `libraryItems` to be replaced/merged with current library |
...
function App() {
  ...
  useEffect(() => {
    ...
    drawinkAPI.updateScene({ appState: { openSidebar: "library" } });
  }, [drawinkAPI]);
  ...
  return (
    <div style={{ height: "500px" }}>
      ...
      <Drawink
        drawinkAPI={(api) => setDrawinkAPI(api)}
        // initial data retrieved from https://github.com/drawink/drawink/blob/master/dev-docs/packages/drawink/initialData.js
        initialData={{
...
<pre>
  (files:{" "}
  <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L59">
    BinaryFileData
  </a>
  ) => void
</pre>
...
<pre>
  () =>{" "}
  <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/element/types.ts#L115">
    DrawinkElement[]
  </a>
</pre>
...
<pre>
  () =>{" "}
  <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L95">
    AppState
  </a>
</pre>
...
```tsx
(
  target?: DrawinkElement | DrawinkElement[],
  opts?:
      | {
...
| --- | --- | --- | --- |
| target | [DrawinkElement](https://github.com/drawink/drawink/blob/master/packages/drawink/element/types.ts#L115) &#124; [DrawinkElement[]](https://github.com/drawink/drawink/blob/master/packages/drawink/element/types.ts#L115) | All scene elements | The element(s) to scroll to. |
...
Updates the `offsets` for the `Drawink` component so that the coordinates are computed correctly (for example the cursor position).

You don't have to call this when the position is changed on page scroll or when the drawink container resizes (we handle that ourselves).

For any other cases if the position of drawink is updated (example due to scroll on parent container and not page scroll) you should call this API.
...
The unique id of the drawink component. This can be used to identify the drawink component, for example importing the library items to the drawink component from where it was initiated when you have multiple drawink components rendered on the same page as shown in [multiple drawink demo](https://codesandbox.io/s/multiple-drawink-k1xx5).

## getFiles

<pre>
  () =>{" "}
  <a href="https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L82">
    files
  </a>
</pre>
...
| Name | Type | Default | Description |
| --- | --- | --- | --- |
| `type` | [ToolType](https://github.com/drawink/drawink/blob/master/packages/drawink/types.ts#L91) | `selection` | The tool type which should be set as active tool |
...
This API is especially useful when you render a custom [`<Sidebar/>`](/docs/@drawink/drawink/api/children-components/sidebar), and you want to toggle it from your app based on a user action.
...
Subscribes to change events, similar to [`props.onChange`](/docs/@drawink/drawink/api/props#onchange).
