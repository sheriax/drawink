...
import type { DrawinkArrowElement, NonDeleted } from "@/lib/elements/types";
...
describe("move element", () => {
  it("rectangle", async () => {
    const { getByToolName, container } = await render(<Drawink />);
  ...
  it("rectangles with binding arrow", async () => {
    ...
    act(() => {
      ...
      bindBindingElement(
        arrow.get() as NonDeleted<DrawinkArrowElement>,
        rectA.get(),
        "orbit",
        "start",
        h.app.scene,
    ...
    expect([[arrow.width, arrow.height]]).toCloselyEqualPoints([[79, 132.89433]], 0);
    ...
    expect([[arrow.width, arrow.height]]).toCloselyEqualPoints([[79, 124.1678]], 0);
