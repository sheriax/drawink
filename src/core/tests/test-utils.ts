...
import type { AllPossibleKeys } from "@/lib/common/utility-types";
...
const renderApp: TestRenderFn = async (ui, options) => {
  ...
  Object.defineProperty(GlobalTestState, "canvas", {
    // must be a getter because at the time of DrawinkApp render the
...
export const withDrawinkDimensions = async (
...
export const getCloneByOrigId = <T extends boolean = false>(
  ...
  return null as T extends true ? DrawinkElement | null : DrawinkElement;
};
 ...
 */
export const assertElements = <T extends AllPossibleKeys<DrawinkElement>>(
  actualElements: readonly DrawinkElement[],
  /** array order matters */
  ...
  const selectedElementIds = expectedElementsWithIds.reduce(
    (acc: Record<DrawinkElement["id"], true>, el) => {
      if (el.selected) {
        acc[el.id] = true;
      }
      return acc;
