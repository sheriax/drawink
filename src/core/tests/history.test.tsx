...
import { ElementsDelta, AppStateDelta } from "@/lib/elements";
...
describe("history", () => {
  ...
  describe("multiplayer undo/redo", () => {
    ...
    beforeEach(async () => {
      await render(
        <Drawink handleKeyboardGlobally={true} isCollaborating={true} />,
      );
    });
    ...
    describe("conflicts in arrows and their bindable elements", () => {
      ...
      it("should rebind bindings when both are updated through the history and there are no conflicting updates in the meantime", async () => {
        ...
        expect(
          (h.elements[2] as DrawinkElbowArrowElement).endBinding,
        ).not.toEqual([1, 0.5001]);
        expect(
          (h.elements[2] as DrawinkElbowArrowElement).endBinding?.mode,
        ).toBe("orbit");
