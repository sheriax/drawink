...
export class Pointer {
  ...
  select(
    ...
    elements: DrawinkElement | DrawinkElement[],
  ) {
    API.clearSelection();
...
const transform = (
  element: DrawinkElement | DrawinkElement[],
...
const proxy = <T extends DrawinkElement>(
  element: T,
): typeof element & {
...
export class UI {
  ...
  /**
   * Creates an Drawink element, and returns a proxy that wraps it so that
  ...
  static group(elements: DrawinkElement[]) {
  ...
  static ungroup(elements: DrawinkElement[]) {
    mouse.select(elements);
    Keyboard.withModifierKeys({ ctrl: true, shift: true }, () => {
      Keyboard.keyPress(KEYS.G);
    });
