...
describe("search", () => {
  beforeEach(async () => {
    await render(<Drawink handleKeyboardGlobally />);
    API.setAppState({
      openSidebar: null,
    });
  });
  ...
  it("should match text split across multiple lines", async () => {
    ...
    API.updateElement(h.elements[0] as DrawinkTextElement, {
      text: "t\ne\ns\nt \nt\ne\nx\nt \ns\np\nli\nt \ni\nn\nt\no\nm\nu\nlt\ni\np\nl\ne \nli\nn\ne\ns",
  ...
  it("should match frame names", async () => {
    ...
    API.updateElement(h.elements[0] as DrawinkFrameLikeElement, {
