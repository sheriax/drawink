...
import { charWidth } from "@/lib/elements";
import { containsCJK } from "@/lib/elements";
...
export class Fonts {
  ...
  private static async loadFontFaces(
    fontFamilies: Array<DrawinkTextElement["fontFamily"]>,
  ...
  private static init() {
    ...
    const init = (
      ...
      ...fontFacesDescriptors: DrawinkFontFaceDescriptor[]
    ) => {
      const fontFamily =
        FONT_FAMILY[family as keyof typeof FONT_FAMILY] ??
        FONT_FAMILY_FALLBACKS[family as keyof typeof FONT_FAMILY_FALLBACKS];
  ...
  private static getUniqueFamilies(
    elements: ReadonlyArray<DrawinkElement>,
  ): Array<DrawinkTextElement["fontFamily"]> {
    return Array.from(
...
export interface DrawinkFontFaceDescriptor {
  uri: string;
