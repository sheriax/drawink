import { promiseTry, LOCAL_FONT_PROTOCOL } from "@/lib/common";

import { subsetWoff2GlyphsByCodepoints } from "../subset/subset-main";

type DataURL = string;

export class DrawinkFontFace {
  ...
  constructor(family: string, uri: string, descriptors?: FontFaceDescriptors) {
    this.urls = DrawinkFontFace.createUrls(uri);

    const sources = this.urls
      .map((url) => `url(${url}) ${DrawinkFontFace.getFormat(url)}`)
      .join(", ");
  ...
  private static createUrls(uri: string): URL[] | DataURL[] {
    ...
    // absolute assets paths, which are found in tests and apps/web build, won't work with base url, so we are stripping initial slash away
    ...
    urls.push(new URL(assetUrl, DrawinkFontFace.ASSETS_FALLBACK_URL));
