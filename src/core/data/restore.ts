...
import { normalizeFixedPoint } from "@/lib/elements";
import { updateElbowArrowPoints, validateElbowPoints } from "@/lib/elements";
...
import { detectLineHeight } from "@/lib/elements";
import {
...
import { refreshTextDimensions } from "@/lib/elements";

import { getNormalizedDimensions } from "@/lib/elements";
...
import type { MarkOptional, Mutable } from "@/lib/common/utility-types";
...
const repairBinding = <T extends DrawinkArrowElement>(
  element: T,
  ...
  if (isElbowArrow(element)) {
    const fixedPointBinding:
      | DrawinkElbowArrowElement["startBinding"]
      | DrawinkElbowArrowElement["endBinding"] = {
      ...binding,
...
const restoreElementWithProperties = <
  T extends Required<Omit<DrawinkElement, "customData">> & {
    customData?: DrawinkElement["customData"];
    /** @deprecated */
    boundElementIds?: readonly DrawinkElement["id"][];
    /** @deprecated */
    strokeSharpness?: StrokeRoundness;
  },
  K extends Pick<T, keyof Omit<Required<T>, keyof DrawinkElement>>,
>(
  element: T,
  extra: Pick<
    T,
    ...
    keyof K
  > &
    Partial<Pick<DrawinkElement, "type" | "x" | "y" | "customData">>,
): T => {
  const base: Pick<T, keyof DrawinkElement> = {
...
export const restoreElement = (
  element: Exclude<DrawinkElement, DrawinkSelectionElement>,
  elementsMap: Readonly<ElementsMap>,
  opts?: {
    ...
    case "arrow": {
      ...
      const base = {
        ...
        startBinding: repairBinding(
          element as DrawinkArrowElement,
        ...
        x,
        y,
        elbowed: (element as DrawinkArrowElement).elbowed,
        ...getSizeFromPoints(points),
      };
...
const repairContainerElement = (
  container: Mutable<DrawinkElement>,
...
const repairFrameMembership = (
  element: Mutable<DrawinkElement>,
  elementsMap: Map<string, Mutable<DrawinkElement>>,
) => {
  if (element.frameId) {
