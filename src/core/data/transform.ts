...
import { measureText, normalizeText } from "@/lib/elements";
import { isArrowElement } from "@/lib/elements";

import { syncInvalidIndices } from "@/lib/elements";

import { redrawTextBoundingBox } from "@/lib/elements";

import { LinearElementEditor } from "@/lib/elements";

import { getCommonBounds } from "@/lib/elements";

import { Scene } from "@/lib/elements";

import type { ElementConstructorOpts } from "@/lib/elements";
...
import type { MarkOptional } from "@/lib/common/utility-types";
...
const bindTextToContainer = (
  ...
  const textElement: DrawinkTextElement = newTextElement({
    x: 0,
    y: 0,
...
const bindLinearElementToElement = (
  linearElement: DrawinkArrowElement,
  start: ValidLinearElement["start"],
  end: ValidLinearElement["end"],
  elementStore: ElementStore,
  scene: Scene,
): {
  linearElement: DrawinkLinearElement;
  startBoundElement?: DrawinkElement;
  endBoundElement?: DrawinkElement;
} => {
  ...
  if (start) {
    ...
    if (startType) {
      ...
      bindBindingElement(
        linearElement,
        startBoundElement as DrawinkBindableElement,
        "orbit",
        "start",
        scene,
      );
  ...
  if (end) {
    ...
    if (endType) {
      ...
      bindBindingElement(
        linearElement,
        endBoundElement as DrawinkBindableElement,
        "orbit",
        "end",
        scene,
      );
...
export const convertToDrawinkElements = (
  elementsSkeleton: DrawinkElementSkeleton[] | null,
  ...
  const elementsWithIds = new Map<string, DrawinkElementSkeleton>();
  ...
  for (const element of elements) {
    ...
    switch (element.type) {
      ...
      case "arrow": {
        ...
        Object.assign(drawinkElement, getSizeFromPoints(drawinkElement.points));
        break;
      }
      case "text": {
  ...
  for (const [id, element] of elementsWithIds) {
    ...
    if (!frame) {
      throw new Error(`Drawink element with id ${id} doesn't exist`);
    }
    const childrenElements: DrawinkElement[] = [];
