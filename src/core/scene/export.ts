...
const truncateText = (element: DrawinkTextElement, maxWidth: number) => {
...
const addFrameLabelsAsTextElements = (
  ...
  const nextElements: NonDeletedDrawinkElement[] = [];
  for (const element of elements) {
    if (isFrameLikeElement(element)) {
      let textElement: Mutable<DrawinkTextElement> = newTextElement({
        x: element.x,
...
const prepareElementsForRender = ({
  ...
  exportingFrame: DrawinkFrameLikeElement | null | undefined;
  frameRendering: AppState["frameRendering"];
  exportWithDarkMode: AppState["exportWithDarkMode"];
}) => {
  let nextElements: readonly DrawinkElement[];
