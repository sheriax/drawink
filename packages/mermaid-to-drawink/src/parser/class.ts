...
// This is the offset to update the arrow head postition for rendering in drawink as mermaid calculates the position until the start of arrowhead
...
const getStrokeStyle = (type: number) => {
  let lineType: DrawinkLinearElement["strokeStyle"];
  switch (type) {
    case LINE_TYPE.LINE:
      lineType = "solid";
      break;
...
const getArrowhead = (type: RELATION_TYPE_VALUES) => {
  let arrowhead: DrawinkLinearElement["startArrowhead"];
  switch (type) {
