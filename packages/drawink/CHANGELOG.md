...
We've transitioned from `UMD` to `ESM` bundle format. Our new `dist` folder inside `@drawink/excalidraw` package now contains only bundled source files, making any dependencies tree-shakable. The package comes with the following structure:
...
import * as excalidrawLib from "@drawink/excalidraw";
// excalidraw react component
import { Drawink } from "@drawink/excalidraw";
// excalidraw styles, usually auto-processed by the build tool (i.e. vite, next, etc.)
import "@drawink/excalidraw/index.css";
// excalidraw types (optional)
import type { DrawinkImperativeAPI } from "@drawink/excalidraw/types";
```
...
<link
  rel="stylesheet"
  href="https://esm.sh/@drawink/excalidraw@0.18.0/dist/dev/index.css"
/>
...
<script type="module">
  ...
  import * as DrawinkLib from "https://esm.sh/@drawink/excalidraw@0.18.0/dist/dev/index.js?external=react,react-dom";
</script>
```
...
// after
import { CaptureUpdateAction } from "@drawink/excalidraw";
updateScene({
  elements,
  appState,
...
- `DrawinkTextElement.baseline` was removed and replaced with a vertical offset computation based on font metrics, performed on each text element re-render. In case of custom font usage, extend the `FONT_METRICS` object with the related properties. [#7693](https://github.com/excalidraw/excalidraw/pull/7693)

- `DrawinkEmbeddableElement.validated` was removed and moved to the private editor state. This should largely not affect your apps unless you were reading from this attribute. We keep validating embeddable urls internally, and the public [`props.validateEmbeddable`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/props#validateembeddable) still applies. [#7539](https://github.com/excalidraw/excalidraw/pull/7539)
...
- Prefer user defined coordinates and dimensions when creating a frame using [`convertToDrawinkElements`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/excalidraw-element-skeleton#converttoexcalidrawelements) [#8517](https://github.com/excalidraw/excalidraw/pull/8517)

- `props.initialData` can now be a function that returns `DrawinkInitialDataState` or `Promise<DrawinkInitialDataState>` [#8107](https://github.com/excalidraw/excalidraw/pull/8135)
...
- Drawink issue #9045 flowcharts: align attributes of new node [#9047](https://github.com/excalidraw/excalidraw/pull/9047)
...
- Docker build of Drawink app [#7430](https://github.com/excalidraw/excalidraw/pull/7430)
...
- Add safe check for arrow points length in tranformToDrawinkElements [#7863](https://github.com/excalidraw/excalidraw/pull/7863)
...
- Export types for @drawink/utils [#7736](https://github.com/excalidraw/excalidraw/pull/7736)
...
- Keep customData when converting to DrawinkElement. [#7656](https://github.com/excalidraw/excalidraw/pull/7656)
...
- Support `excalidrawAPI` prop for accessing the Drawink API [#7251](https://github.com/excalidraw/excalidraw/pull/7251).

- Export [`getCommonBounds`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/utils#getcommonbounds) helper from the package [#7247](https://github.com/excalidraw/excalidraw/pull/7247).
...
- The `Ref` support has been removed in v0.17.0 so if you are using refs, please update the integration to use the [`excalidrawAPI`](http://localhost:3003/docs/@drawink/excalidraw/api/props/excalidraw-api) [#7251](https://github.com/excalidraw/excalidraw/pull/7251).
...
- [`useDevice`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/utils#usedevice) hook's return value was changed to differentiate between `editor` and `viewport` breakpoints. [#7243](https://github.com/excalidraw/excalidraw/pull/7243)
...
- Added [`props.validateEmbeddable`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/props#validateembeddable) to customize embeddable src url validation. [#6691](https://github.com/excalidraw/excalidraw/pull/6691)
- Add support for `opts.fitToViewport` and `opts.viewportZoomFactor` in the [`DrawinkAPI.scrollToContent`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/props/excalidraw-api#scrolltocontent) API. [#6581](https://github.com/excalidraw/excalidraw/pull/6581).
...
- Use subdirectory for @drawink/excalidraw size limit [#6787](https://github.com/excalidraw/excalidraw/pull/6787)
...
- [`DrawinkAPI.scrollToContent`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/props/excalidraw-api#scrolltocontent) has new opts object allowing you to fit viewport to content, and animate the scrolling. [#6319](https://github.com/excalidraw/excalidraw/pull/6319)

- Expose `useI18n()` hook return an object containing `t()` i18n helper and current `langCode`. You can use this in components you render as `<Drawink>` children to render any of our i18n locale strings. [#6224](https://github.com/excalidraw/excalidraw/pull/6224)

- [`restoreElements`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/utils/restore#restoreelements) API now takes an optional parameter `opts` which currently supports the below attributes

```js
{ refreshDimensions?: boolean, repairBindings?: boolean }
```

The same `opts` param has been added to [`restore`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/utils/restore#restore) API as well.

For more details refer to the [docs](https://docs.excalidraw.com)

#### BREAKING CHANGE

- The optional parameter `refreshDimensions` in [`restoreElements`](https://docs.excalidraw.com/docs/@drawink/excalidraw/api/utils/restore#restoreelements) has been removed and can be enabled via `opts`
...
- Division by zero in findFocusPointForEllipse leads to infinite loop in wrapText freezing Drawink [#6377](https://github.com/excalidraw/excalidraw/pull/6377)
...
- Expose component API for the Drawink main menu [#6034](https://github.com/excalidraw/excalidraw/pull/6034), You can read more about its usage [here](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#MainMenu)

- Support customization for the Drawink [main menu](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#MainMenu) [#6034](https://github.com/excalidraw/excalidraw/pull/6034).
...
- Any top-level children passed to the `<Drawink/>` component that do not belong to one of the officially supported Drawink children components are now rendered directly inside the Drawink container (previously, they weren't rendered at all) [#6096](https://github.com/excalidraw/excalidraw/pull/6096).
...
- `DrawinkArrowElement` rather than `DrawinkArrowEleement` [#5955](https://github.com/excalidraw/excalidraw/pull/5955)
...
- Added support for storing [`customData`](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#storing-custom-data-on-excalidraw-elements) on Drawink elements [#5592].
...
- Added exportPadding to PNG (blob) export in @drawink/utils [#5626](https://github.com/excalidraw/excalidraw/pull/5626)

- Introduce DrawinkElements and DrawinkAppState provider [#5463](https://github.com/excalidraw/excalidraw/pull/5463)
...
- Add display name for Drawink component so it doesn't show as anonymous [#5464](https://github.com/excalidraw/excalidraw/pull/5464)
...
- Prevent browser zoom inside Drawink [#5426](https://github.com/excalidraw/excalidraw/pull/5426)
...
- Expose [`serializeAsJSON`](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#serializeAsJSON) helper that we use when saving Drawink scene to a file [#3538](https://github.com/excalidraw/excalidraw/pull/3538).
...
- [`scrollToContent`](https://github.com/excalidraw/excalidraw/blob/master/src/components/App.tsx#L265) API now supports passing just a single Drawink element, or none at all (which will default to current elements on the scene) [#3482](https://github.com/excalidraw/excalidraw/pull/3482).
...
- Make history local to a given Drawink instance. This fixes a case where history was getting shared when you have multiple Drawink components on the same page [#3481](https://github.com/excalidraw/excalidraw/pull/3481).
- Use active Drawink component when editing text. This fixes a case where text editing was not working when you have multiple Drawink components on the same page [#3478](https://github.com/excalidraw/excalidraw/pull/3478).

- Fix library being pasted off-center [#3462](https://github.com/excalidraw/excalidraw/pull/3462).

- When switching theme, apply it only to the active Drawink component. This fixes a case where the theme was getting applied to the first Drawink component if you had multiple Drawink components on the same page [#3446](https://github.com/excalidraw/excalidraw/pull/3446).
...
- Export types for the package so now it can be used with typescript [#3337](https://github.com/excalidraw/excalidraw/pull/3337). The types are available at `@drawink/excalidraw/types`.
...
- Add `name` prop to indicate the name of the drawing which will be used when exporting the drawing. When supplied, the value takes precedence over `intialData.appState.name`, the `name` will be fully controlled by host app and the users won't be able to edit from within Drawink [#3273](https://github.com/excalidraw/excalidraw/pull/3273).
- Export API `setCanvasOffsets` via `ref` to set the offsets for Drawink[#3265](https://github.com/excalidraw/excalidraw/pull/3265).
  #### BREAKING CHANGE
...
- Add a `theme` prop to indicate Drawink's theme. [#3228](https://github.com/excalidraw/excalidraw/pull/3228). When this prop is passed, the theme is fully controlled by host app.
...
- Don't overflow text beyond width of Drawink [#3215](https://github.com/excalidraw/excalidraw/pull/3215).
...
- Expose `window.EXCALIDRAW_ASSET_PATH` which host can use to load assets from a different URL. By default it will be loaded from `https://unpkg.com/@drawink/excalidraw{currentVersion}/dist/`[#3068](https://github.com/excalidraw/excalidraw/pull/3068).
...
- Support Drawink inside scrollable container [#3018](https://github.com/excalidraw/excalidraw/pull/3018)
...
- Add view mode in Drawink [#2840](https://github.com/excalidraw/excalidraw/pull/2840)
...
- Add safe check for process so Drawink can be loaded via script [#2824](https://github.com/excalidraw/excalidraw/pull/2824)
...
- Exported few [Extra API's](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#extra-apis) which can be used by the host to communicate with Drawink.

- Remove language picker, and add `langCode`, `renderFooter` [#2644](https://github.com/excalidraw/excalidraw/pull/2644):
  - BREAKING: removed the language picker from UI. It is now the host app's responsibility to implement a language picker if desirable, using the newly added [`renderFooter`](https://github.com/excalidraw/excalidraw/blob/master/src/packages/excalidraw/README.md#renderFooter) prop. The reasoning is that the i18n should be controlled by the app itself, not by the nested Drawink component.
...
First release of `@drawink/excalidraw`
