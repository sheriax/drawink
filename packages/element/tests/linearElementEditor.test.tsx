import { pointCenter, pointFrom } from "@drawink/math";
...
} from "../../drawink/tests/queries/dom";
...
describe("Test Linear Elements", () => {
  ...
  beforeEach(async () => {
    ...
    reseed(7);
    const comp = await render(<Drawink handleKeyboardGlobally={true} />);
    h.state.width = 1000;
    h.state.height = 1000;
  ...
  const createTwoPointerLinearElement = (
    ...
    roundness: DrawinkElement["roundness"] = null,
    roughness: DrawinkLinearElement["roughness"] = 0,
  ) => {
  ...
  it("should not drag line and add midpoint until dragged beyond a threshold", () => {
    ...
    const line = h.elements[0] as DrawinkLinearElement;
  ...
  it("should allow dragging line from midpoint in 2 pointer lines outside editor", async () => {
    ...
    expect((h.elements[0] as DrawinkLinearElement).points.length).toEqual(2);
  ...
  it("shouldn't create text element on double click in line editor (arrow)", async () => {
    createTwoPointerLinearElement("arrow");
    const arrow = h.elements[0] as DrawinkLinearElement;
  ...
  describe("Inside editor", () => {
    ...
    it("should update the midpoints when element roundness changed", async () => {
      ...
      const midPointsWithRoundEdge = LinearElementEditor.getEditorMidPoints(
        h.elements[0] as DrawinkLinearElement,
        h.app.scene.getNonDeletedElementsMap(),
        h.state,
      );
    ...
    describe("When edges are round", () => {
      ...
      it("should allow dragging lines from midpoints in between segments", async () => {
        ...
        expect((h.elements[0] as DrawinkLinearElement).points)
  ...
  describe("Test bound text element", () => {
    ...
    const createBoundTextElement = (
      text: string,
      container: DrawinkLinearElement,
    ) => {
    ...
    it("should bind text to arrow when double clicked", async () => {
      ...
      expect(
        (h.elements[1] as DrawinkTextElementWithContainer).text,
      ).toMatchSnapshot();
    });
    ...
    it("should bind text to arrow when clicked on arrow and enter pressed", async () => {
      ...
      const textElement = h.elements[1] as DrawinkTextElementWithContainer;
    ...
    it("should wrap the bound text when arrow bound container moves", async () => {
      ...
      const textElement = h.elements[2] as DrawinkTextElementWithContainer;
    ...
    it("should not update label position when arrow dragged", () => {
      createTwoPointerLinearElement("arrow");
      let arrow = h.elements[0] as DrawinkLinearElement;
      createBoundTextElement(DEFAULT_TEXT, arrow);
      let label = h.elements[1] as DrawinkTextElementWithContainer;
      ...
      arrow = h.elements[0] as DrawinkLinearElement;
      label = h.elements[1] as DrawinkTextElementWithContainer;
